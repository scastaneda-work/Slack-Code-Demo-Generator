...
Before a live demo, vet the configured persona end-to-end:

```sh
.venv/bin/python qa.py --self-test       # offline fixtures, sub-second
.venv/bin/python qa.py --vet-questions   # live: runs each question in examples/demo_questions.md through run_agent()
```

`--self-test` checks the Block Kit extractor and validator against fixtures
covering every parse tier and common schema violations. `--vet-questions`
fires each demo question through the agent and checks for Block Kit
parseability, schema violations, and character-break markers ("as an AI",
"who are you?", etc.). Exit code is non-zero on any failure.
